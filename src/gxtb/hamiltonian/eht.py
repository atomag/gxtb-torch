...
Code deviations / extensions:
    ...
    - f shells included structurally; integrals default to zeros until implemented.

Limitations / TODO:
    - Analytical gradients (Eq. 68–71) not yet implemented.
...
def eht_energy_gradient(
    ...
    *,
    ...
    """Analytic gradient of E^{EHT} = Tr(P H^{EHT}) excluding ∂Ŝ^{sc}/∂R (Step A).

    Implements Eqs. 68–71 terms:
      - ∂H_{l}/∂R via CN (Eq. 69) using dCN/dR (Eq. 48b)
      - ∂Π/∂R via distance polynomials (Eqs. 70–71)
    The ∂Ŝ^{sc}/∂R term (Eq. 39) is not included here and should be added in Step B.
    """
