...
def test_scf_reports_mfx_energy_for_CH():
    dtype = torch.float64
    ...
    # Assemble MFX parameters (no exponential screening, no R0 needed)
    Zmax = max(gparams.elements)
    U_shell = torch.zeros((Zmax+1, 4), dtype=dtype, device=device)
    # Assign moderate values for elements present
